...
var ClassNameBuilder = function () {
    function ClassNameBuilder(options) {
        this.b_ = b_1.B(options);
    }
    ClassNameBuilder.prototype.stringify = function (block, elem, mods, mixes, cls) {
        var _this = this;
        return this.b_(block, elem, mods) + (mixes ? ' ' + mixes.map(function (mix) {
            return _this.b_(mix.block || block, mix.elem, mix.mods);
        }).join(' ') : '') + (cls ? ' ' + cls : '');
    };
    ClassNameBuilder.prototype.joinMixes = function (mix1, mix2) {
        ...
        var mixes = [];
        mix1 && (mixes = mixes.concat(mix1));
        mix2 && (mixes = mixes.concat(mix2));
        return mixes;
    };
